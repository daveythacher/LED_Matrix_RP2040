...
No matter how many sub PWM periods are used the result should be mostly free of flicker. However the finer detail is lost in the process potentially. Assuming you have 12 bit PWM period with 6.25MHz GCLK at 30 FPS then you would still have 11-bits at 3kHz. Note this requires the sub PWM periods to be smaller or equal to 11-bits. There may be total of 16-bits possible, however only 11-bits will be used. This configuration would be 11+5. Which means there 32 11-bit PWM periods.

It should also be noted that S-PWM uses a very high multiplexing rate if using a S-PWM configuration like 6+10, which has an impact on ghosting. It is preferred to match the desired sub PWM period size to the worst case effective PWM bits per frame. Otherwise the multiplexing rate will be too high. This will have an effect on ghosting. It may also not be possible in all cases.
