...
static uint16_t index_table[256][6];
...
static void __not_in_flash_func(set_pixel)(uint8_t x, uint8_t y, uint8_t r0, uint8_t g0, uint8_t b0, uint8_t r1, uint8_t g1, uint8_t b1) {
    ...
    buf[bank][y][x % 16][3][x / 16] = index_table[r0][3];
    buf[bank][y][x % 16][4][x / 16] = index_table[g0][4];
    buf[bank][y][x % 16][5][x / 16] = index_table[b0][5];
}
