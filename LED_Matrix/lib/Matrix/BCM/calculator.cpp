...
namespace Matrix::Calculator {
    ...
    static constexpr void is_brightness_valid() {
        ...
        constexpr float brightness = ((period_us / get_refresh_overhead()) - (led_rise_us * PWM_bits / 2.0)) / period_us;
        constexpr float accuracy = ((period_us / get_refresh_overhead()) - (led_rise_us * PWM_bits / 2.0)) / (period_us / get_refresh_overhead());
    ...
    static constexpr void is_blank_time_valid() {
        constexpr float led_fall_us_high = (10000 * COLUMNS * max_led_cap_pf) / 1000000.0;
