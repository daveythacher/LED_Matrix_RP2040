...
namespace Matrix {
    ...
    static volatile struct {volatile uint32_t len; volatile uint8_t *data;} address_table[(1 << lower_bits) + 2];
    ...
    */
    ...
    void start() {
        ...
        for (uint32_t i = 0; i < (1 << lower_bits); i++)
            address_table[i].len = COLUMNS + 1;

        address_table[1 << lower_bits].data = null_table;
        address_table[1 << lower_bits].len = COLUMNS + 1;
        address_table[(1 << lower_bits) + 1].data = NULL;
        address_table[(1 << lower_bits) + 1].len = 0;
        
        // Hack to lower the ISR tick rate, accelerates by 2^lower_bits (Improves refresh performance)
        ...
        /*while (1) {
            counter2 = (1 << lower_bits) - 1; LAT = 0;    // Start of frame, manually push into FIFO (data stream protocol)
            do {
    ...
    void __not_in_flash_func(send_line)() {
        ...
        pio_sm_put(pio0, 0, 1 << lower_bits);
