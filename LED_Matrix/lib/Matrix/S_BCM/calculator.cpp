...
namespace Matrix::Calculator {
    ...
    static constexpr void is_clk_valid() {
        ...
        constexpr double clk_hz = hz_limit / (MIN_REFRESH * get_refresh_overhead() * COLUMNS * MULTIPLEX * (1 << lower_bits));
    ...
    void verify_configuration() {
        ...
        static_assert((MULTIPLEX * COLUMNS * (1 << upper_bits) * lower_bits) <= (48 * 1024), "The current buffer size is not supported");
