...
void matrix_start() {
    ...
    */
    ...
    static_assert(GCLK <= 33000000.0, "Code limits Grayscale clock for TLC5948 to 33MHz");
    ...
    // GCLK SM
    pio1->sm[0].clkdiv = ((uint32_t) floor(G_CLK) << 16) | ((uint32_t) round((G_CLK - floor(G_CLK)) * 255.0) << 8);
